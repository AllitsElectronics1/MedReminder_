...
const int eveningLED = 15;        // GPIO pin for evening LED
const int buzzerPin = 14;         // GPIO pin for buzzer
...
void checkAlertStatus(int ledPin) {
  ...
  if ((millis() - TimerStartTime) < ThresholdTime)
  {
    ...
    { 
      if (randomTimerStarted && (millis() - randomTimerStartTime) <= randomTimerDuration) // Lid closed within random timer //if <10s
      {
...
void sendMedicineStatus(String macAddress,String message,String status) {
  HTTPClient http;
  http.begin("http://192.168.0.101:8080/api/devices/updateMedicineStatus");
...
bool registerDevice(String macAddress, String ipAddress) {
  HTTPClient http;
  http.begin("http://192.168.0.101:8080/api/devices/update-ip"); // Initialize HTTP client with server URL
